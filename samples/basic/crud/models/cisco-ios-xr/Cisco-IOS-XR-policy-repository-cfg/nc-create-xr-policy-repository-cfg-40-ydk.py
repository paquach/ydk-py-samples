...
usage: nc-create-xr-policy-repository-cfg-40-ydk.py [-h] [-v] device name status
...
def config_routing_policy(routing_policy, name, status):
    """Add config data to routing_policy object."""
    route_policy_name = name
    rpl_route_policy = """
        route-policy """ + name + """
          """ + status + """
        end-policy
...
if __name__ == "__main__":
    ...
    config_routing_policy(routing_policy, args.name, args.status)  # add object configuration
